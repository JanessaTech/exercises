...
describe('MultiSigWallet', function () {
    async function deployMultiSigWalletFixture() {
        ...
        return { multiSigWallet, account1, account2, account3, account4, account5, account6, data}
    }
    ...
    describe('submitTransaction', function () {
        it('It should submit a new transaction succesfully when it is owner', async function () {
            const {multiSigWallet, account1, account5, data} = await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
        ...
        it('It should fail to submit a new transaction when it is not owner', async function () {
            const {multiSigWallet,  account5, account6, data} = await loadFixture(deployMultiSigWalletFixture)
            await expect(multiSigWallet.connect(account6).submitTransaction(account5.address, data)).to.be.revertedWith('only owner')
        })
    })
    describe('confirmTransaction', function () {
        ...
        it('It fails to confirm the tx when it does not exist', async function () {
            const {multiSigWallet, data, account5} = await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
        ...
        it('It fails to confirm the tx when it is executed', async function () {
            const {multiSigWallet, account2, account3, account4, account5, account6, data} =await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
            await multiSigWallet.confirmTransaction(0)
    ...
    describe('revokeConfirm', function () {
        ...
        it('it fails to revoke when it does not exist', async function () {
            const {multiSigWallet, account5, data} =await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
        ...
        it('It fails to revoke when it is already executed', async function () {
            const {multiSigWallet, account2, account3, account5, data} =await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
            await multiSigWallet.confirmTransaction(0)
            await multiSigWallet.connect(account2).confirmTransaction(0)
            await multiSigWallet.connect(account3).confirmTransaction(0)
    ...
    describe('executeTransaction', function () {
        it('It fails to execute when it is not owner', async function () {
            const {multiSigWallet, data,  account2, account3,  account5, account6} = await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
            await multiSigWallet.confirmTransaction(0)
            await multiSigWallet.connect(account2).confirmTransaction(0)
            await multiSigWallet.connect(account3).confirmTransaction(0)
        ...
        it('It fails to execute when it does not exist', async function () {
            const {multiSigWallet, data,  account2, account3, account5} = await loadFixture(deployMultiSigWalletFixture)
            await multiSigWallet.submitTransaction(account5.address, data)
            await multiSigWallet.confirmTransaction(0)
            await multiSigWallet.connect(account2).confirmTransaction(0)
            await multiSigWallet.connect(account3).confirmTransaction(0)
