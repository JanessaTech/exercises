 ...
 * 
 * operations.gql
query GetPoolLiveData($poolId: ID!) @live(interval: 5000) {
...
query GetMultiplePoolsLiveData($poolIds: [ID!]!) @live(interval: 5000) {
...
async function main() {
  ...
  const poolIds = ["0xe0554a476a092703abdb3ef35c80e0d76d32939f", 
    ...
    // chainId=42161
    /*
    const poolIds = ["0xc35ab4ee32198d3f7e82f9e5aa66daafb7a73c6e",
  ...
  const poolIds = ["0xc35ab4ee32198d3f7e82f9e5aa66daafb7a73c6e",
                    ...
                    "0xc473e2aee3441bf9240be85eb122abb059a3b57c",
                    "0x42fc852a750ba93d5bf772ecdc857e87a86403a9"]
    const multiplePoolsliveDataStream   = await sdk.GetMultiplePoolsLiveData({poolIds: poolIds})
    try {
      for await (const { pools } of multiplePoolsliveDataStream) {
        console.log(`[${new Date().toISOString()}] update pool data:`, pools);
        // trigger bussiness codes
      }
    } catch (error) {
      console.error('Found error. Polling stopped:', error);
