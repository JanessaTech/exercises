...
async function main() {
  ...
  const poolIds_ethereum = ["0xe0554a476a092703abdb3ef35c80e0d76d32939f", 
    ...
    // 启动对Ethereum上池子的监听
  const ethereumStream = await ethereum_sdk.GetMultipleEthereumPoolLiveData({ poolIds: poolIds_ethereum });
  (async () => {
   ...
   */
    ...
    const poolIds_arbitrum = ["0xc35ab4ee32198d3f7e82f9e5aa66daafb7a73c6e",
      ...
      const arbitrumStream = await arbitrum_sdk.GetMultipleArbitrumPoolLiveData({ poolIds: poolIds_arbitrum });
      (async () => {
